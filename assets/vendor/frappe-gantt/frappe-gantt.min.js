var Gantt = (function () {
  ...
  var h = {
    ...
    format(t, e = "YYYY-MM-DD HH:mm:ss.SSS", s = "en") {
      const i = this.get_date_values(t).map((t) => d(t, 2, 0)),
        n = {
          YYYY: i[0],
          MM: d(+i[1] + 1, 2, 0),
          DD: i[2],
      ...
      return (
        Object.keys(n)
          ...
          .forEach((t) => {
            a.includes(t) && ((a = a.replace(t, `$${o.length}`)), o.push(n[t]));
          }),
        o.forEach((t, e) => {
          a = a.replace(`$${e}`, t);
        }),
    ...
    add(r, h, d) {
      h = parseInt(h, 10);
      const p = [
        r.getFullYear() + (d === t ? h : 0),
        r.getMonth() + (d === e ? h : 0),
        r.getDate() + (d === s ? h : 0),
        r.getHours() + (d === i ? h : 0),
        r.getMinutes() + (d === n ? h : 0),
        r.getSeconds() + (d === a ? h : 0),
        r.getMilliseconds() + (d === o ? h : 0),
      ];
      return new Date(...p);
    },
    start_of(r, h) {
      const d = { [t]: 6, [e]: 5, [s]: 4, [i]: 3, [n]: 2, [a]: 1, [o]: 0 };
      function p(t) {
        return d[t] <= d[h];
      }
      const _ = [
  ...
  class u {
    ...
    prepare_values() {
      (this.invalid = this.task.invalid),
        ...
        (this.duration =
          h.diff(this.task._end, this.task._start, "hour") /
    ...
    draw_bar() {
      (this.$bar = _("rect", {
        ...
        g(this.$bar, "width", 0, this.width),
        this.invalid && this.$bar.classList.add("bar-invalid");
    }
    draw_progress_bar() {
      this.invalid ||
        ((this.$bar_progress = _("rect", {
          x: this.x,
          y: this.y,
    ...
    draw_resize_handles() {
      ...
      _("rect", {
        ...
        this.task.progress &&
          ...
          (this.$handle_progress = _("polygon", {
    ...
    show_popup() {
      ...
      const t =
        h.format(this.task._start, "MMM D", this.gantt.options.language) +
        " - " +
        h.format(
    ...
    date_changed() {
      ...
      Number(this.task._start) !== Number(e) &&
        ...
        Number(this.task._end) !== Number(s) &&
          ((t = !0), (this.task._end = s)),
        t &&
  ...
  class f {
    ...
    setup_wrapper(t) {
      ...
      e
        ...
        : (this.$svg = _("svg", { append_to: s, class: "gantt" })),
    ...
    setup_tasks(t) {
      (this.tasks = t.map((t, e) => {
        if (
          ((t._start = h.parse(t.start)),
          (t._end = h.parse(t.end)),
          h.diff(t._end, t._start, "year") > 10 && (t.end = null),
          (t._index = e),
          !t.start && !t.end)
        ) {
          const e = h.today();
          (t._start = e), (t._end = h.add(e, 2, "day"));
        }
        !t.start && t.end && (t._start = h.add(t._end, -2, "day")),
          t.start && !t.end && (t._end = h.add(t._start, 2, "day"));
        if (
          (h
            .get_date_values(t._end)
            .slice(3)
            .every((t) => 0 === t) && (t._end = h.add(t._end, 24, "hour")),
    ...
    setup_gantt_dates() {
      ...
      (this.gantt_start = h.start_of(this.gantt_start, "day")),
        (this.gantt_end = h.start_of(this.gantt_end, "day")),
        this.view_is([w.QUARTER_DAY, w.HALF_DAY])
          ? ((this.gantt_start = h.add(this.gantt_start, -7, "day")),
            (this.gantt_end = h.add(this.gantt_end, 7, "day")))
          : this.view_is(w.MONTH)
          ? ((this.gantt_start = h.start_of(this.gantt_start, "year")),
            (this.gantt_end = h.add(this.gantt_end, 1, "year")))
          : this.view_is(w.YEAR)
          ? ((this.gantt_start = h.add(this.gantt_start, -2, "year")),
            (this.gantt_end = h.add(this.gantt_end, 2, "year")))
          : ((this.gantt_start = h.add(this.gantt_start, -1, "month")),
            (this.gantt_end = h.add(this.gantt_end, 1, "month")));
    }
    setup_date_values() {
      this.dates = [];
      let t = null;
      for (; null === t || t < this.gantt_end; )
        (t = t
          ? this.view_is(w.YEAR)
            ...
            : this.view_is(w.MONTH)
            ? h.add(t, 1, "month")
            : h.add(t, this.options.step, "hour")
          : h.clone(this.gantt_start)),
          this.dates.push(t);
    }
    bind_events() {
    ...
    setup_layers() {
      ...
      for (let e of t)
        this.layers[e] = _("g", { class: e, append_to: this.$svg });
    }
    make_grid() {
    ...
    make_grid_rows() {
      const t = _("g", { append_to: this.layers.grid }),
        e = _("g", { append_to: this.layers.grid }),
    ...
    make_grid_ticks() {
      ...
      for (let i of this.dates) {
        let n = "tick";
        this.view_is(w.DAY) && 1 === i.getDate() && (n += " thick"),
          this.view_is(w.WEEK) &&
            i.getDate() >= 1 &&
            i.getDate() < 8 &&
            (n += " thick"),
          ...
          this.view_is(w.MONTH)
            ? (t += (h.get_days_in_month(i) * this.options.column_width) / 30)
    ...
    make_grid_highlights() {
      if (this.view_is(w.DAY)) {
        _("rect", {
          x:
            (h.diff(h.today(), this.gantt_start, "hour") / this.options.step) *
      ...
      const i = {
          ...
          Week_lower:
            t.getMonth() !== e.getMonth()
              ? h.format(t, "D MMM", this.options.language)
              : h.format(t, "D", this.options.language),
          ...
          Day_upper:
            t.getMonth() !== e.getMonth()
              ? h.format(t, "MMMM", this.options.language)
              : "",
          Week_upper:
            t.getMonth() !== e.getMonth()
              ? h.format(t, "MMMM", this.options.language)
          ...
          Month_upper:
            t.getFullYear() !== e.getFullYear()
              ? h.format(t, "YYYY", this.options.language)
              : "",
          Year_upper:
            t.getFullYear() !== e.getFullYear()
              ? h.format(t, "YYYY", this.options.language)
      ...
      for (let t of this.tasks) {
        ...
        (e = t.dependencies
          .map((e) => {
            ...
            if (!s) return;
            const i = new c(this, this.bars[s._index], this.bars[t._index]);
    ...
    set_scroll_position() {
      ...
      if (!t) return;
      const e =
        (h.diff(this.get_oldest_starting_date(), this.gantt_start, "hour") /
          this.options.step) *
          this.options.column_width -
        this.options.column_width;
      t.scrollLeft = e;
    ...
    bind_bar_events() {
      ...
      (this.bar_being_dragged = null),
        p.on(this.$svg, "mousedown", ".bar-wrapper, .handle", (r, h) => {
          const d = p.closest(".bar-wrapper", h);
          h.classList.contains("left")
            ? (i = !0)
            : h.classList.contains("right")
            ? (n = !0)
            : h.classList.contains("bar-wrapper") && (t = !0),
            d.classList.add("active"),
            (e = r.offsetX),
            (s = r.offsetY),
            (a = d.getAttribute("data-id"));
          const _ = [a, ...this.get_all_dependent_tasks(a)];
          (o = _.map((t) => this.get_bar(t))),
    ...
    bind_bar_progress() {
      let t = 0,
        ...
        n = null,
        a = null;
      p.on(this.$svg, "mousedown", ".handle.progress", (o, r) => {
        (s = !0), (t = o.offsetX), (e = o.offsetY);
        const h = p.closest(".bar-wrapper", r).getAttribute("data-id");
        (i = this.get_bar(h)),
          (n = i.$bar_progress),
          (a = i.$bar),
          (n.finaldx = 0),
        ...
        p.on(this.$svg, "mousemove", (e) => {
          ...
          p.attr(n, "width", n.owidth + a),
            p.attr(o, "points", i.get_progress_polygon_points()),
            (n.finaldx = a);
        }),
